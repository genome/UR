...
$rule = UR::BoolExpr->resolve_for_class_and_params('URT::43Primary', primary_id => 1, primary_value => 'One');
ok($rule, 'Create rule');
$value = $context->infer_property_value_from_rule('primary_id', $rule);
is($value, 1, 'get a value directly in the rule');


$rule = UR::BoolExpr->resolve_for_class_and_params('URT::43Primary', rel_id => 1);
ok($rule, 'Create rule');
$value = $context->infer_property_value_from_rule('primary_id', $rule);
...
$rule = UR::BoolExpr->resolve_for_class_and_params('URT::43Primary', related_value => '1');
ok($rule, 'Create rule');
...
$rule = UR::BoolExpr->resolve_for_class_and_params('URT::43Primary', related_value => '2');
ok($rule, 'Create rule');
...
#$rule = UR::BoolExpr->resolve_for_class_and_params('URT::43Primary', rel_id => 3);
#ok($rule, 'Create rule');
#$value = $context->infer_property_value_from_rule('related_value', $rule);
...
$rule = UR::BoolExpr->resolve_for_class_and_params('URT::43Related', related_id => 2);
ok($rule, 'Create rule');
