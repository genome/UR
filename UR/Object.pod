...
application layer just requests objects crom the current "context", according to 
...
The get() method lets the context internally decidewhether to return a cached 
...
example in the synopsisis continued for deeper illustration.
...
will be in an invalid state until correct, and will not be commitable.
...
Regular accessors have the same name as the property, as declared, and work as is commonly expected:
...
simply match zero items, ...or fail to understand your parameters), and exception is used.
