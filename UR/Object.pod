...
UR::Object is the base class which provides several general features
to all UR-based classes.  Most classes tracked through UR's object system
will inherit from UR::Object.

The goal of UR::Objects is that your application doesn't have to do 
...
Make a new instance of SomeClass.  This instance will not be visable outside
the application until it is committed to its data source.

=item define

  $obj = SomeClass->define(...);

Creates a new instance of SomeClass within the application.  define()
differs from create() in that define() simply asserts that the specified
object exists in the current Context, not that it is a newly created object.

Objects that exist because of define(), and whose properties have not been
altered since their definition, will not be saved outside the application.
...
UR::Object::Iterator instances are normal hashrefs, not UR-based objects,
and obey the same scoping rules as any normal Perl reference. 
...
=item get_class_object

  $class_obj = $obj->get_class_object();
