...
and then define the datasource. We specify the data source's name with the
--dsname argument, and a DBI connect string with the --dsn argument. For
this example, we'll use a brand new SQLite database. For some other, perhaps
already existing database, give its connect string instead.

  ur define datasource rdbms --dsname Example --dsn dbi:SQLite:Example.sqlite3

which generates this output:

  A   CdExample::DataSource::Example (UR::DataSource::SQLite)
     ...connecting...
     ....ok
...
Again, the correcponding database rows will not be removed until you commit.
