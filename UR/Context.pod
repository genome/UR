...
subclassing, then additional importing iterators is created to handle that.
Finally, the object against the rule's class is returned to the caller.
...
_sync_databases() is the workhorse behind L</rollback>.  
...
difference between UR and other Perl ORMs.  Other systems do no caching
and require the calling application to hold references to any objects it 
...
=head2 Delted Objects and Ghosts
...
is class names.  The second level is rule IDs.  The values is how many times
that class and rule have been involved in a get().  This data is used by
L</_loading_was_done_before_with_a_superset_of_this_params_hashref> to 
