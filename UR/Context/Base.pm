...
sub _initialize_for_current_process {
    ...
    my $context_singleton_class = $ENV{UR_CONTEXT_BASE} ||= 'UR::Context::DefaultBase';
...
sub get_current {
    ...
    #eval "sub get_current { \$ENV{UR_CONTEXT_BASE} }";
    return $ENV{UR_CONTEXT_BASE};
}
...
sub set_current {
    ...
    return $value if $value eq $ENV{UR_CONTEXT_BASE};
    
    $ENV{UR_CONTEXT_BASE} = $value;
    ...
    unless ($ENV{UR_CONTEXT_BASE}->isa("UR::Context")) {
        die "The context at application initialization is set to "
            . $ENV{UR_CONTEXT_BASE} . ".\n"
            . "This does not inherit from UR::Context."
    }
    
    unless ($ENV{UR_CONTEXT_BASE}->get_class_object) {
        die "The context at application initialization is set to "
            . $ENV{UR_CONTEXT_BASE} . ".\n"
