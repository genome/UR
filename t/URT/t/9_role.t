...
subtest 'parameterized role' => sub {
    ...
    use URT;
    our $the_length : RoleParam(the_length);
    role ParameterizedRole {
        has => [
            role_prop => { is => 'Text', len => $the_length },
        ],
    };
    sub the_length { $the_length }
    sub anon_sub_with_the_length { return sub { $the_length } }

    package main;
    isa_ok($ParameterizedRole::the_length,
            'UR::Role::Param',
            'Before being composed, role param');

    foreach my $class_data ( [ 'ClassWithParameterizedRole', 999 ], ['AnotherClassWithParameterizedRole', 5432 ] ) {
        my($class_name, $role_param_value) = @$class_data;

        UR::Object::Type->define(
            class_name => $class_name,
            roles => [ ParameterizedRole->create(the_length => $role_param_value) ],
        );
        ...
        is_deeply($roles->[0]->role_params,
                    { the_length => $role_param_value },
                    'Role instance params');

        is($class_name->__meta__->property('role_prop')->data_length,
            $role_param_value,
            'Role property metadata was filled in with the role param value');

        is($class_name->the_length,
            $role_param_value,
        ...
        TODO: {
            ...
            my $sub = $class_name->anon_sub_with_the_length();
            lives_and { is($sub->(),
                            $role_param_value,
    ...
    throws_ok {
        class ClassWithTooManyRoleParams {
            roles => [ ParameterizedRole->create(the_length => 111, bogus_param => 222) ],
        } }
    ...
    throws_ok {
        ...
        qr(Role ParameterizedRole expects values for these params: the_length),
