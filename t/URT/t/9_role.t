...
subtest 'parameterized role' => sub {
    ...
    isa_ok($ParameterizedRole::the_length,
            'UR::Role::Param',
            'Before being composed, role param is a UR::Role::Param');

    package ClassWithParameterizedRole;
    my %custom_role_params = ( the_length => 999 );

    class ClassWithParameterizedRole {
        roles => [ ParameterizedRole->create(%custom_role_params) ],
    };

    package main;
    my $roles = ClassWithParameterizedRole->__meta__->roles;
    is(scalar(@$roles), 1, 'Class has 1 roles');
    isa_ok($roles->[0], 'UR::Role::Instance');
    is_deeply($roles->[0]->role_params, \%custom_role_params, 'Role instance params');
};
