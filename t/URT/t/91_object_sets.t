...
do {
    my $urt_person_init_subclass = 0;
    *URT::Person::_init_subclass = sub { $urt_person_init_subclass = 1 };
    my $set_class = 'URT::Person::Set';
    my $initializer = $set_class->can('_init_subclass');
    $@ = '';
    eval { $initializer->($set_class, $set_class) };
    my $error = $@;
    is($error, '', 'no error when calling _init_subclass on set class');
    is($urt_person_init_subclass, 1, '_init_subclass was run for URT::Person when calling _init_subclass on URT::Person::Set');
};
