...
sub dynamically_load_class {
    ...
    my $meta;
    if ($namespace->isa("UR::Namespace")) {
        # dynamic loading under a UR namespace gets more explicit control of what loads
        unless ($namespace->should_dynamically_load_class($class)) {
            return;
        }
        
        # attempt to get a class object, loading it as necessary (probably).
        $meta = $namespace->get_member_class($class);
        unless ($meta) {
            return;
        }

        # verify that we only auto-load modules from inside the namespace's module tree
        ...
        # we don't _require_ that you make a UR namespace above the class, 
        # but you will get less control and performance w/o it 
        $meta = UR::Object::Type->get($class);
        unless ($meta) {
            return;
        }
    }
    ...
    # It only occurs when a class object is made directly in code.
