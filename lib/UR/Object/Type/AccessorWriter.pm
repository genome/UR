...
sub mk_ro_class_accessor {
    ...
    my $accessor = Sub::Name::subname $full_accessor_name => sub {
        if (@_ > 1) {
            ...
            if ($different or $@ =~ m/has no overloaded magic/) {
                Carp::croak("Cannot change read-only class-wide property $accessor_name for class $class_name from $variable_value to $new!");
            }
        }
        return $variable_value;
    };
...
=item mk_ro_class_accessor
  ...
  $classobj->mk_ro_class_accessor($class_name, $accessor_name, $column_name, $is_transient, $variable_value);
