...
sub _sql_like_escape_string {
    return q('\\');  # Most RDBMSs support an 'escape' as part of a 'like' operator, excapt mysql
}
...
sub _extend_sql_for_column_operator_and_value {
    ...
    if ($op eq '' or $op eq '=' or $op eq 'eq') {
        ...
        $sql .= "$expr_sql $op ?";
        push @sql_params, $val;        
        if($op =~ /like/i and my $db_escape = $self->_sql_like_escape_string) {
            $escape ||= $db_escape;
            $sql .= " escape $escape";
        }
    } elsif($op =~ /^(ne|\!\=)$/i) {                
