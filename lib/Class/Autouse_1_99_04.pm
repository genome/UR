...
BEGIN {
    if ($INC{"Class/Autouse.pm"}) {
        die "Attempt to use Class::Autouse_1_99_04 when older Class::Autouse is in place.  Use the new one first!";
    }
    else {
        # ensure attempts to use the old version are ignored
        $INC{"Class/Autouse.pm"} = $INC{"Class/Autouse_1_99_04.pm"};
        $INC{"Class/Autouse_1_99_02.pm"} = $INC{"Class/Autouse_1_99_04.pm"};
    }
}
